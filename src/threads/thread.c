...
// function to perform the donation logic
void donate_priority(struct thread *donee)
{
  struct thread *t = thread_current();
  ASSERT(donee != NULL);
  ...
  while (donee != NULL && depth < 8)
  {
    ...
    if (donee != t->donee)
    {
      // Add the current thread to the donee's donor list
      list_push_back(&donee->donar_list, &t->donar_elem);
    }

    // update the priority of the donee thread to be equal the priority of the current thread
    donee->priority = t->priority;

    // set the donee of current thread to the current donee
    // this created a chain of priority inheritance for the coming up donation
    // this will enable us to move down the chain of donees
    t->donee = donee;
...
void thread_set_priority(int new_priority)
{
  ...
  enum intr_level old_level;
  old_level = intr_disable();
  // the function ensures that it only updates the priority if it is not increased by donations.
  //  or if the new priority is higher than the current priority
  //  then it update the priority to the new value
  if (t->priority == t->base_priority || new_priority > t->priority)
    t->priority = new_priority;

  // update the base priority as well
  t->base_priority = new_priority;

  intr_set_level(old_level);

  // find the maximum priority thread in the ready list
  struct list_elem *max_elem = list_min(&ready_list, cmp_priority, NULL);
  int max_priority = list_entry(max_elem, struct thread, elem)->priority;

  // if the current thread's priority is lower than the maximum priority
  // in the ready list yield the CPU to a higher priority thread
...
int
calc_mlfqs_priority(fixedpoint recent_cpu, int nice){
...
int
calc_recent_cpu(fixedpoint recent_cpu, int nice)
{
    fixedpoint calc = multiply_by_int(load_avg, 2);
    calc = divide_by_real(calc, add_to_int(calc, 1));
    calc = multiply_by_real(calc, recent_cpu);
    calc = add_to_int(calc, nice);
    return calc;
}
void
update_mlfqs_equ(){
...
next_thread_to_run(void)
{
  ...
  else
  {
    ...
    // find the next thread with the highest priority from the ready list and remove it from the ready list
    struct list_elem *t = list_min(&ready_list, cmp_priority, NULL);
    list_remove(t);
